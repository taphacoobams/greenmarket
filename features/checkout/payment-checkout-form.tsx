...
export function PaymentCheckoutForm() {
  ...
  useEffect(() => {
    if (!draft) router.replace("/checkout");
  }, [draft, router]);
  ...
  function pay(values: PaymentValues) {
    ...
    window.setTimeout(() => {
      ...
      clearDraft();
      setProgress(100);
      router.push(`/order-confirmation?ref=${encodeURIComponent(order.id)}`);
    }, 1350);
  }

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <form onSubmit={form.handleSubmit(pay)} className="mx-auto flex max-w-2xl flex-col gap-10">
        <div className="rounded-[2rem] border border-border bg-card p-6 shadow-sm space-y-2">
          ...
          <p className="pt-4 text-lg font-bold text-primary">{formatCurrency(totalDue)} à acquitter</p>
        </div>
